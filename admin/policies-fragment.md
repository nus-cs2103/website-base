...
==#r#Do not ask tutors admin questions##== (in person or via other means) as **tutors are prohibited from answering admin questions**.
...
When working with others, especially in a large class such as {{ course_pair }}, it is very important that you adhere to standards, policies, and instructions imposed on everyone. Not doing so creates unnecessary headaches for everyone and puts your work attitude in a negative light. That is why we penalize repeated non-compliance with instructions. On the other hand, we do understand that we are all liable to make mistakes. That is why we only penalize repeated or frequent mistakes.

</div>

<div id="policy-workDistribution">

As most of the work is graded individually, it is OK to do less or more than an equal share in your project team.
...
* **Let us know only if you failed to earn full marks for participation due to such a reason** (or if you feel you are at risk of not earning full marks), in which case we will consider giving an alternative way to earn the marks missed because of the absences.<br>
 Details of how participation marks are calculated are [here]({{ baseUrl }}/admin/participation.html).
...
* **If you are covered by an MC during an F2F tutorial but you are well enough to attend it via Zoom**, you may request to attend the tutorial via Zoom only, by writing to `{{ course_email }}` -- in such cases, Zoom-only attendance can be counted for participation (in contrast, Zoom-only attendance of F2F tutorials is not counted for participation under normal circumstances).
 {% endif %}
</div>

<div id="policy-responseTime">

**Normally, the prof will respond within 24 hours** to an email sent to the prof or a forum post directed at the prof. If you don't get a response within that time, **please feel free to remind the prof**. It is likely that the prof did not notice your post or the email got stuck somewhere.
...
The promptness of a reply is even more important when the email is requesting something that you cannot provide. Imagine you wrote to the prof requesting a reference letter and the prof did not respond at all because they did not want to give you one; you'll be quite frustrated because you wouldn't know whether to look for another prof or wait longer for a response. Saying 'No' is fine and in fact a necessary part of professional life; but saying nothing is not acceptable. If you didn't reply, the sender will not even know whether you received the email.
...
**It is OK to ask for help from classmates even for assignments**, even from other teams, as long as you don't copy from others and submit it as your own. The preferred way to do this is to ask in the [forum]({{ url_forum }}) rather than privately, as (1) it gives others an opportunity to benefit as well, and (2) the teaching team can confirm the accuracy of the help you are receiving.
...
Penalties: NUS policy dictates that all plagiarism cases are to be reported and handled centrally (i.e., at school and university level), not by course lecturers. Hence, the penalty will be determined by the personnel handling the investigation at faculty/university level.
...
* You are allowed to reuse work from your classmates or past students, subject to the following conditions:
  * The work has been shared publicly by us or the authors.
  * You clearly give credit to the original author(s).
* You are allowed to reuse code from external sources, subject to the following conditions:
  * You clearly give credit to the original author/source.
  * You do not violate the license under which the work has been released. Please **do not use third-party images/audio** in your software unless they have been specifically released to be used freely. Just because you found it on the Internet does not mean it is free for reuse.
...
**Automated plagiarism checks ahead!**{.text-danger} We'll be using automated plagiarism checks to detect uncredited reuse of content from other {{ course }} tPs, i.e., tPs done by other teams in this batch and ==_all_ previous batches==. These checks will be done _after_ the final submissions. The cases detected will be verified and reported to the university administration for disciplinary action. As you know, NUS enforces a penalty of `F` grade for the entire course (with no option to S/U or withdraw from the course) for plagiarism offenses.
</box>
<box>
...
**referred to code in another software** or<br>
...
* If you copy-pasted code from elsewhere but modified it significantly, mention it as a comment<br>
  ...
  {Your implementation of the reused solution here ...}
  ```
* If you copy-pasted a non-trivial code block (possibly with minor modifications %%e.g., renaming, layout changes, changes to comments, etc.%%), also mark the code block as reused code (using <trigger trigger="click" for="modal:reusePolicy-authorTags">`@@author` tags</trigger> with the <mark><code>-reused</code></mark> suffix)<br>
  Format:
