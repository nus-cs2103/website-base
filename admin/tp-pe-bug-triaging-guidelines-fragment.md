...
* **If the bug is specific to an OS**: The bug that appears only on a specific OS is still a valid bug, given that the app is required to work on all three supported OSes.<br>
...
* **Handling manual edits to the data file**: AB3 UG specifies [the current level of support for manually editing the data file](https://se-education.org/addressbook-level3/UserGuide.html#editing-the-data-file) i.e., 'if you edit the file correctly, things will work; but if you edited it wrongly, you can lose all data'. At least that level of support should be provided in the product as well. Bugs can be filed in the following cases:<br>
