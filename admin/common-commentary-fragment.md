...
<!-- ------------------------------------------------------------------------------------------------------ -->
<div id="followup-{{ slugify("Code Quality: Coding Standards") }}">

{% call show_ai_impact("Code Quality, Coding Standards") %}
**AI changes who applies a coding standard, not who needs to understand it.**

* **Applying the rules by hand matters much less now.** Indentation, brace placement, line length: IDEs already did most of this, and an agent will do the rest. Time spent arguing over brace styles was never well spent, and it is worth even less today.
* **Consistency itself matters more, not less.** You now read far more code than you write, and much of it was written by an agent rather than a teammate. ==An agent also takes its cues from the code already there, so a tidy codebase tends to stay tidy, while a messy one gets messier with every change.== Because generated code arrives in bulk, that drift happens fast.
* **Tools can check the form of your code, not the sense of it.** A <tooltip content="a type of tool that can check if the code follows specific style rules e.g., Checkstyle ">linter</tooltip> can confirm that a name is in camelCase. It cannot tell you that the name is misleading. Choosing names that mean something, writing comments that explain _why_, keeping a method short enough to follow: these are judgment calls, and they are  elements of code quality that matter even more.
* **You still need to know the coding standard and other good code hygiene practices, because you are the one accepting the code.** If you cannot recognize a code quality issue, you cannot tell whether what the agent produced is good enough.
{% endcall %}
</div>
<!-- ------------------------------------------------------------------------------------------------------ -->
