...
Accordingly, the tP is structured to resemble a non-trivial real-life brownfield software project where you will:
1. conceptualize and implement enhancements to a given product, and
1. prepare it to be continued by future developers
</div>
<div tags="m--cs2113">
...
The tP focuses on the following aspects:
1. ~~coding~~ %%(taken for granted, not focused on specifically)%%
1. working in a team
1. process/workflow
1. documentation
1. scheduling and tracking project progress, meeting delivery deadlines
1. quality assurance
...
The final product should target users who can type fast and prefer typing over mouse/voice commands. Therefore, ==typed user commands are the primary mode of input.==
...
**Narrow down only to the extent you intend to optimize**: Narrowing down the target user should be tied to both the intention and the feasibility of optimizing the application for that 'narrowed down' target user. For example, it doesn't make sense to narrow down the target user to _NUS students_ if the application will not be optimized for NUS students specifically (i.e., the app can be used irrespective of which university the user is from).
</box>
...
* **Define a clear _value proposition_** that matches the target user profile: what problem does the product solve? How does it make the user's life easier?<br>
